...
struct fraq{
	...
	fraq(char c): start(new state(c)){}
	bool operator!=(int c){
	    return start->ch != c;
	}
};

void linkOperator(string re, int n, int i, fraq tmp, stack<fraq>& ans, stack<char>& ops){
    fraq tmp1;
    if(i!=n-1 && re[i+1]!='*' && re[i+1]!='+' && re[i+1]!='?' && re[i+1]!='|' && re[i+1]!=')'){
        while(!ans.empty() && ans.top()!='(' && !ops.empty() && ops.top()=='#'){
            tmp1 = ans.top() ;
            ans.pop();
            tmp1.out.clear();
            tmp1.out.splice(tmp1.out.end(), tmp.out);
            ops.pop();   // delete #
            tmp = tmp1;
        }
        ans.push(tmp);
            ...
            case '|':
                ...
                while(ans.top().ch!='(' && !ops.empty()){
                    ...
                    if(ops.top() == '#'){   // #
                        ...
                        s = new state(128, tmp2.start, tmp1.start);
                        tmp2.out.splice(tmp2.out.end(), tmp1.out);
                        tmp2.start = s;
                    }
                    ops.pop();
